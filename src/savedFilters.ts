...
export type PreparedFilterDraft = Omit<SavedFilter, "id">;

export type PrepareFilterDraftResult =
  | { ok: true; filter: PreparedFilterDraft }
...
export function savedFilterLabel(f: Pick<SavedFilter, "query" | "kinds" | "statuses">): string {
...
/** Turn the direct-builder fields into the existing saved-filter model. The
 *  parser remains the source of filter semantics; this boundary only rejects
 *  an all-empty draft and a duplicate set of conditions, neither of which is a
 *  useful new saved filter. Names continue to be generated by
 *  `savedFilterLabel`, exactly like filters saved from the top bar. */
export function prepareFilterDraft(
  draft: FilterDraft,
  existing: SavedFilter[],
): PrepareFilterDraftResult {
  const filter: PreparedFilterDraft = {
    query: draft.query.trim(),
    kinds: [...draft.kinds],
    statuses: [...draft.statuses],
    color: draft.color.toLowerCase(),
    opacity: normalizeFilterOpacity(draft.opacity),
    highlight: draft.highlight,
  };
  if (!filter.query && filter.kinds.length === 0 && filter.statuses.length === 0) {
    return { ok: false, error: "Add a query, resource type, or status before saving." };
  }
  if (!HEX_COLOR.test(filter.color)) {
    return { ok: false, error: "Choose a valid six-digit highlight color." };
  }

  const duplicate = existing.find(
    (saved) =>
      saved.query.trim() === filter.query &&
      sameValues(saved.kinds, filter.kinds) &&
      sameValues(saved.statuses, filter.statuses),
  );
  if (duplicate) {
    return {
      ok: false,
      error: `This filter is already saved as “${savedFilterLabel(duplicate)}”.`,
    };
  }

  // Parse eagerly at the boundary so builder-created filters always take the
  // same grammar path as the filter bar and saved-filter matching pipeline.
  parseFilter(filter.query);
  return { ok: true, filter };
}

/** Whether this query needs the backend-backed traffic matcher. */
...
export interface FilterMatches {
  ...
  /** filter id → matching-row count; null when this legacy pure helper
   *  cannot evaluate the filter without a backend scan. */
